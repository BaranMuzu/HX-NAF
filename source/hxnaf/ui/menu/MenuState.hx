...
class MenuStateOriginal extends FlxState
{
  // Variables
  ...
  override public function update(elapsed:Float)
  {
    ...
    if (FlxG.mouse.justPressed)
    {
      if (FlxG.mouse.overlaps(newGame)) clickPlay();

      if (FlxG.mouse.overlaps(fiveHitbox))
      {
        titleClick++;
        flipsound.play(true);
        if (!evilxel) gameTitle.loadGraphic("assets/images/mainmenu/texts/ONE_GAME_TITLE.png");
  ...
  function clickPlay()
  {
    openSubState(new NewspaperSubState());
  }
}
